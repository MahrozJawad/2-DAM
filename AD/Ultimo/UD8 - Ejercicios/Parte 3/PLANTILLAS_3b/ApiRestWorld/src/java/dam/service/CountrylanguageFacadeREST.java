...
import dam.Countrylanguage;
import dam.CountrylanguagePK;
...
public class CountrylanguageFacadeREST extends AbstractFacade<Countrylanguage> {
    ...
    private CountrylanguagePK getPrimaryKey(PathSegment pathSegment) {
         ...
         * URI path part is supposed to be in form of 'somePath;countrycode=countrycodeValue;languagename=languagenameValue'.
         * Here 'somePath' is a result of getPath() method invocation and
         * it is ignored in the following code.
         * Matrix parameters are used as field names to build a primary key instance.
         */
        dam.CountrylanguagePK key = new dam.CountrylanguagePK();
        javax.ws.rs.core.MultivaluedMap<String, String> map = pathSegment.getMatrixParameters();
        java.util.List<String> countrycode = map.get("countrycode");
        if (countrycode != null && !countrycode.isEmpty()) {
            key.setCountrycode(countrycode.get(0));
        }
        java.util.List<String> languagename = map.get("languagename");
        if (languagename != null && !languagename.isEmpty()) {
            key.setLanguagename(languagename.get(0));
        }
        return key;
    }
    ...
    public void remove(@PathParam("id") PathSegment id) {
        dam.CountrylanguagePK key = getPrimaryKey(id);
