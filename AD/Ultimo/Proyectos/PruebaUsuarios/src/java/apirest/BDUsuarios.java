...
public class BDUsuarios {
    ...
    public static Usuarios userget(String id) {  
        ...
        if (d==null) {
            ...
            user.setEdad(d.getDouble("edad").toString());
    ...
    // PUT - Idioma y edad
    public static boolean userPutIdiomaYEdad(Usuarios user) {
        
        try {
            collection.updateOne(Filters.eq("id",user.getId()), Updates.set("idioma",user.getIdioma()));         
            collection.updateOne(Filters.eq("id",user.getId()), Updates.set("edad",user.getEdad())); 
        } catch (Exception e) {
            return false;
        }
        return true;
