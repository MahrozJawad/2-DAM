package RedSocial03Actualizaciones;
...
public class RedSocial03Actualizaciones {
    ...
    public static void QuitarCamposFecha() {
        ...
        System.out.println("Se han eliminado los campos 'fecha' correctamente.");
    }

    public static void AñadirCamposCantidad_amigos() {
        for (Document d : collection.find()) {
            if (d.containsKey("amigos")) {
                ArrayList<Integer> amigos = (ArrayList<Integer>) d.get("amigos");
                collection.updateOne(
                        Document.parse(
                                "{"
                                + "\"_id\":" + d.getInteger("_id")
                                + "}"
                        ),
                        Document.parse(
                                "{"
                                + "$set:{\"cantidad_amigos\":" + amigos.size() + "}"
                                + "}"
                        ));
            }
        }
        System.out.println("Añadido el campo 'cantidad de amigos'");
    }

    public static void AñadirComentarioADaniela() {
        Scanner e = new Scanner(System.in);
        System.out.print("Introduce el título:: ");
        String titulo = e.nextLine();
        System.out.print("Introduce el comentariio: ");
        String texto = e.nextLine();
        System.out.print("Introduce el grupo: ");
        String grupo = e.nextLine();
        Document comentario = new Document("titulo", titulo)
                .append("texto", texto)
                .append("fecha", new Date())
                .append("grupo", grupo);
        collection.updateOne(
                Document.parse("{\"nombre\":\"Daniela\"}"),
                Document.parse(
                        "{$push:{\"comentarios\":" + comentario.toJson() + "}}"
                ));
        ...
        System.out.println("Se ha incrementado el valor de total_comentarios correctamente.");
    }
